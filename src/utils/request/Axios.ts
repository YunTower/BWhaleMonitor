...
export class VAxios {
   ...
   * @param options
   */
  request<T = unknown>(config: AxiosRequestConfigRetry, options?: RequestOptions): Promise<T> {
    ...
    if (
      requestOptions &&
      requestOptions.throttle &&
      requestOptions.throttle.delay !== undefined &&
      requestOptions.throttle.delay !== 0
    ) {
    ...
    if (
      ...
      requestOptions.debounce.delay !== undefined &&
      requestOptions.debounce.delay !== 0
    ) {
      return new Promise((resolve) => {
        debounce(
          () => resolve(this.synthesisRequest(config, options)),
   ...
   * @private
   */
  private async synthesisRequest<T = unknown>(
